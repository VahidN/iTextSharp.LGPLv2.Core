...
{
    private static readonly byte[] _rotbits =
    {
    ...
    public override PdfStream GetFullFontStream()
    {
        if (Cff)
        {
            return new StreamFont(ReadCffFont(), subType: "CIDFontType0C", compressionLevel);
        }
    ...
    public override int GetWidth(int char1)
    {
        ...
        if (FontSpecific)
        {
            ...
            {
                return GetRawWidth(char1 & 0xff, name: null);
            }
    ...
    public override int GetWidth(string text)
    {
        ...
        if (FontSpecific)
        {
            ...
            for (var k = 0; k < len; ++k)
            {
                ...
                {
                    total += GetRawWidth(c & 0xff, name: null);
                }
            }
        }
        else
    ...
    internal static string ToHex(int n)
    {
        if (n < 0x10000)
        {
            return "<" + Convert.ToString(n, toBase: 16).PadLeft(totalWidth: 4, paddingChar: '0') + ">";
        }
        ...
        return "[<" + Convert.ToString(high, toBase: 16).PadLeft(totalWidth: 4, paddingChar: '0') +
               Convert.ToString(low, toBase: 16).PadLeft(totalWidth: 4, paddingChar: '0') + ">]";
    }
    ...
    internal override void WriteFont(PdfWriter writer, PdfIndirectReference piref, object[] parms)
    {
        ...
        AddRangeUni(longTag, includeMetrics: true, subset);
        ...
        if (Cff)
        {
            ...
            pobj = new StreamFont(b, subType: "CIDFontType0C", compressionLevel);
        ...
        else
        {
            ...
            if (subset || DirectoryOffset != 0)
            {
                var sb = new TrueTypeFontSubSet(FileName, new RandomAccessFileOrArray(Rf), longTag, DirectoryOffset,
                    includeCmap: false, includeExtras: false);
    ...
    private PdfDictionary getCidFontType2(PdfIndirectReference fontDescriptor, string subsetPrefix, object[] metrics)
    {
        ...
        cdic.Put(PdfName.Registry, new PdfString(value: "Adobe"));
        cdic.Put(PdfName.Ordering, new PdfString(value: "Identity"));
        cdic.Put(PdfName.Supplement, new PdfNumber(value: 0));
        ...
        if (!_vertical)
        {
            ...
            for (var k = 0; k < metrics.Length; ++k)
            {
                ...
                if (m == lastNumber + 1)
                {
                    buf.Append(value: ' ').Append(metric[1]);
                }
                else
                {
                    if (!firstTime)
                    ...
                    firstTime = false;
                    buf.Append(m).Append(value: '[').Append(metric[1]);
                }
    ...
    private PdfStream getToUnicode(object[] metrics)
    {
        ...
        var buf = new StringBuilder("/CIDInit /ProcSet findresource begin\n" + "12 dict begin\n" + "begincmap\n" +
                                    "/CIDSystemInfo\n" + "<< /Registry (TTX+0)\n" + "/Ordering (T42UV)\n" +
                                    "/Supplement 0\n" + ">> def\n" + "/CMapName /TTX+0 def\n" + "/CMapType 2 def\n" +
        ...
        for (var k = 0; k < metrics.Length; ++k)
        {
            if (size == 0)
            {
                if (k != 0)
                {
                    buf.Append(value: "endbfrange\n");
                }

                size = Math.Min(val1: 100, metrics.Length - k);
                buf.Append(size).Append(value: " beginbfrange\n");
            }

            --size;
            var metric = (int[])metrics[k];
            var fromTo = ToHex(metric[0]);
            buf.Append(fromTo).Append(fromTo).Append(ToHex(metric[2])).Append(value: '\n');
        }

        buf.Append("endbfrange\n" + "endcmap\n" + "CMapName currentdict /CMap defineresource pop\n" + "end end\n");
        var s = buf.ToString();
        var stream = new PdfStream(PdfEncodings.ConvertToBytes(s, encoding: null));
        stream.FlateCompress(compressionLevel);
