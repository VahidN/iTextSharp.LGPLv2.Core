...
public static class RtfProtection
{
    ...
    public static string GenerateHash(string pwd, string encryptedPwd = "00000000")
    {
        var password = pwd;
        ...
        if (!string.IsNullOrEmpty(password))
        {
            ...
            {
                password = password.Substring(startIndex: 0, length: 15);
            }
            ...
            var s = lo.ToString(format: "x8", CultureInfo.InvariantCulture);
            encryptedPwd = s.Substring(startIndex: 6, length: 2) + s.Substring(startIndex: 4, length: 2);
            s = hi.ToString(format: "x8", CultureInfo.InvariantCulture);
            encryptedPwd += s.Substring(startIndex: 6, length: 2) + s.Substring(startIndex: 4, length: 2);
        }
