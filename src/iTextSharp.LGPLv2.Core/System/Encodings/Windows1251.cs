...
{
    public class Windows1251EncodingProvider : EncodingProvider
    {
        public override Encoding GetEncoding(int codepage)
        {
            return codepage == 1251 ? new Windows1251() : null;
        }

        public override Encoding GetEncoding(string name)
        {
            return name.Equals("windows-1251", StringComparison.OrdinalIgnoreCase) ? new Windows1251() : null;
        }
    }
    ...
    public class Windows1251 : Encoding
    {
        ...
        public char? FallbackCharacter
        {
            ...
            set
            {
                ...
                if (value.HasValue && !_charToByte.ContainsKey(value.Value))
                {
                ...
                FallbackByte = value.HasValue ? _charToByte[value.Value] : (byte?)null;
            }
        }
        ...
        private int getBytesWithFallBack(char[] chars, int charIndex, int charCount, byte[] bytes, int byteIndex)
        {
            for (int i = 0; i < charCount; i++)
            {
                ...
                byte byteValue;
                bool status = _charToByte.TryGetValue(character, out byteValue);
        ...
        private int getCharsWithFallback(byte[] bytes, int byteIndex, int byteCount, char[] chars, int charIndex)
        {
            for (int i = 0; i < byteCount; i++)
            {
                ...
                char result = lookupIndex >= _byteToChar.Length
                                ? FallbackCharacter.Value
                                : _byteToChar[lookupIndex];
        ...
        private int getCharsWithoutFallback(byte[] bytes, int byteIndex, int byteCount, char[] chars, int charIndex)
        {
            ...
            {
                byte lookupIndex = bytes[i + byteIndex];
                if (lookupIndex >= _byteToChar.Length)
                {
                    //throw exception
                ...
                chars[charIndex + i] = _byteToChar[lookupIndex];
            }
      ...
      (char)1103 /* byte 255 */
        ...
        /// </summary>
        private static readonly Dictionary<char, byte> _charToByte = new Dictionary<char, byte>
    {
      { (char)0, 0 },
      { (char)1, 1 },
      { (char)2, 2 },
